...
/**
 * This class is mainly a mistake. When I have time to refactor the precondition
 * mechanism, this class may go to the trash as well.
 * 
...
public class DeferrableClassPrepareRequest extends AbstractDeferrableRequest{
    ...
    private static Map <String, VMRepository> vmTable = new HashMap<String, VMRepository>();
    ...
    private MultiMap<String, String> reqRepository;
    
    
    // Hack to get my static trash working for now
    public static void registerVM(String vmid){
    	vmTable.put(vmid, new VMRepository());
    }

    public DeferrableClassPrepareRequest(String vmid) { 
        super(vmid);
        synchronized(vmTable){
        	reqRepository = vmTable.get(vmid).reqRepository;
        }
    }
    ...
    public Object resolveInternal(IResolutionContext context) throws Exception {
        ...
        synchronized(this){
            ...
            JDIMiscUtil jmt = JDIMiscUtil.getInstance();
    ...
    private static class VMRepository{
        private MultiMap<String, String> reqRepository = 
